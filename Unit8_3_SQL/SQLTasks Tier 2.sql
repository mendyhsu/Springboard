...
SELECT `name` AS 'Facility charge a fee to members'
FROM `Facilities`
WHERE `membercost` > 0
...
SELECT `name` AS 'Facility', `monthlymaintenance` ,
        CASE WHEN `monthlymaintenance` > 100 THEN 'expensive'
             ELSE 'cheap' END AS `Monthly fee type`
FROM `Facilities`
...
SELECT DISTINCT `Facilities`.`name` AS 'Facility', 
...
SELECT `f`.`name` AS 'Facility',
       CASE WHEN `b`.`memid` = 0 THEN `firstname`
            ELSE CONCAT(`firstname`, ' ', `surname`) END AS 'Member Name',
...
LEFT JOIN `Facilities` AS `f`
  USING (`facid`)
WHERE `b`.`starttime` BETWEEN '2012-09-14 00:00:00' AND '2012-09-14 23:59:00'
HAVING `Costs` > 30
ORDER BY `Costs` DESC
...
SELECT `m`.`memid`, 
       CASE WHEN `m`.`memid` = 0 THEN `m`.`surname`
            ELSE CONCAT(`m`.`surname`, ', ', `m`.`firstname`) END AS 'Member Name',
       CASE WHEN `r`.`memid` = 0 THEN `r`.`surname` 
            ELSE CONCAT(`r`.`surname`, ', ', `r`.`firstname`) END AS 'Recommended by'
FROM `Members` AS `m`
LEFT JOIN `Members` AS `r`
  ON `m`.`recommendedby` = `r`.`memid`
WHERE `m`.`memid` NOT IN (0)
ORDER BY `Recommended by`, `Member Name`
...
LEFT JOIN (SELECT `memid`, CONCAT(`surname`, ', ', `firstname`) AS 'Member Name'
           FROM `Members`
           WHERE `memid` NOT IN (0)
           ) AS `subquery2`
  USING (`memid`)
...
FROM `Facilities`
LEFT JOIN (SELECT `facid`, DATE_FORMAT(`starttime`, '%m') AS 'Month',  
